<?php
    ...
    foreach($cart as $item) {
        $form = array($item->name, $item->price, $item->size, $item->qty, $item->sellerId, $orderId);
        $sql = $handler->prepare('INSERT INTO `order-items`(name, price, size, qty, sellerId, orderId) VALUES(?,?,?,?,?,?)');
        $sql->execute($form);
    }
